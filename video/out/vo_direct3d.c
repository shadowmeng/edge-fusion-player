                              ...
                              bool as_texture)
{
    ...
    while (cur->mplayer_fmt) {
        if (cur->mplayer_fmt == movie_fmt) {
            ...
            MP_ERR(priv, "Accepted image format (%s): %s\n",
                   type, vo_format_name(cur->mplayer_fmt));
 ...
 */
typedef int (*func_multid3d_initialize)();
...
{
    d3d_priv *priv = vo->priv;
	MP_ERR(priv, "call control request = %d\n", request);
    switch (request) {
...
static void draw_image(struct vo *vo, mp_image_t *mpi)
{
    ...
    if (!priv->d3d_device)
        goto done;
	MP_ERR(priv, "d3d draw_image w=%d, h=%d fmt=%d IMGFMT_RGBA=%d \n", mpi->w, mpi->h, mpi->imgfmt, IMGFMT_RGBA);
