...
extern const struct vo_driver video_out_multidirect3d;
...
static bool get_desc(struct m_obj_desc *dst, int index)
{
    ...
    *dst = (struct m_obj_desc) {
	...
	//printf("end call get_desc size=%d, index=%s\n", vo->priv_size, vo->name);
    return true;
}
...
struct vo *init_best_video_out(struct mpv_global *global, struct vo_extra *ex)
{
	...
	printf("call init_best_video_out\n");
